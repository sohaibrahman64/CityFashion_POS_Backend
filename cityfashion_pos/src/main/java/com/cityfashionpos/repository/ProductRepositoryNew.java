...
public interface ProductRepositoryNew extends JpaRepository<ProductEntityNew, Long> {
    ...
    // Find by product name starting with search term (case-insensitive)
    @Query("SELECT p FROM ProductEntityNew p WHERE LOWER(p.name) LIKE LOWER(CONCAT(:name, '%'))")
    List<ProductEntityNew> findByNameStartingWithIgnoreCase(@Param("name") String name);
    ...
    List<ProductEntityNew> findByCreatedDateRange(@Param("startDate") String startDate,
